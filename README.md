...
**Opening the page clears the browser's storage for this address.** Local storage, session storage, IndexedDB, the cache store, the origin private file system, and cookies are all emptied before anything else runs, so no leftover from an older version of Number finder can survive anywhere.

This goes wider than Number finder's own data on purpose, so be aware of what shares an address: every page opened from disk shares one, so the offline file clears what other local pages saved, and the website shares one with everything else published at the same address. The page says how many things it cleared and lists them in the console. A browser test seeds all four stores, with Number finder's old names and another page's data, and checks that everything is gone.
...
3. **Search.** Type a number in the search bar (or click any number in a preview) and press Enter. The query stays in the bar, and selecting a search in the history loads it back. Under the bar:
   ...
   - **Search mode** (sums only): which groupings come first. *Clumped (same row first)*, the default, puts numbers that sit together in one file on top, and numbers printed in the same row — one line of a form, one row of a sheet — above everything else; *Spread (within files)* prefers one file but numbers far apart in it; *Across files* prefers one number from each file, like a return line built from several source documents; *Most spread* goes the other way from clumped, preferring numbers from different files, pages and rows. Sums collect more matches than they show first, so the mode has real choices; a check ranks each row's matches the same way.
   ...
   **The bar's grammar** (the empty bar shows these in turn). Words are matched against a number's label, its column and sheet, and its file name, never minding case:
   ...
   | `3,235 sums:3 mode:clumped` | search mode for sums: `mode:clumped` (the default), `mode:spread`, `mode:across`, `mode:scattered` (most spread) |
   ...
   **Order.** With several matches, the *Order* picker above the results offers *Best match* (single numbers, then the ones mentioning a `'word`, then how the numbers sit together — the search mode, so same-row sums first while it is *Clumped* — then fewest numbers, fewest negatives, closest), *Closest to the number typed* (smallest difference first, whatever it is made of), *By file*, and *Document order*.

   **Lots of matches.** A sum search keeps up to 10,000 matches and shows 100 at a time, with *Previous* and *Next* under the list ("Matches 101–200 of 4,312"). Two sums can hold different numbers that read the same — a W-2 has 85,000 in box 1 *and* in box 3, so "85,000 + 2,000 + 3,235" can be two real answers — and when that happens each one says which number it used ("85,000 *1 Wages, tips…*" against "85,000 *3 Social security wages*"). Opening a sum shows every number's file, place, and label in full.
